...
std::string ConstDefAST::astJson(int size) {
    ...
    {
        children.push_back(Json("Index", {ConstExp->astJson(sizeplus(size))}, sizeplus(size)));
        children.push_back(Json("Constant Init", {ConstInitVal->astJson(sizeplus(size))}, sizeplus(size)));
...
llvm::Value *ConstDefAST::codegen() {
    ...
    else//global
    {
        ...
        else//global array
        {
            ...
            for(int j = 0; j < arraySize->getLimitedValue(); j++)
            {
                // arrayValue.push_back(llvm::ConstantInt::get((down->getType(), ConstInitVal->vals[j]->codegen()->getLimitedValue())));
                arrayValue.push_back(llvm::dyn_cast<llvm::Constant>(dynamic_cast<ConstInitValAST*>(ConstInitVal.get())->vals[j]->codegen()));
            }
            llvm::Constant* initValue = llvm::ConstantArray::get(arrayType, arrayValue);
            auto GlobalArray = new llvm::GlobalVariable(
                                               *IR::get()->getModule(),
                                               arrayType,
                                               true,
